...
void main() {
  group('AlertPolicyEngine', () {
    ...
    test('warning events skip sound but keep visible and vibration strategies',
        () {
    ...
    test('explicit requested channels are filtered by global strategy config',
        () {
      final event = _event(
        AlertLevel.warning,
        requestedChannels: const [
      ...
      final channels = const AlertPolicyEngine().resolveChannels(
        ...
        const AlertStrategyConfigSet(
          global: AlertingGlobalConfig(enabled: true),
          sound: SoundAlertConfig(enabled: false),
        ),
      );
