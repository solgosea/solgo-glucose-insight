...
class HighEpisodeReportProvider implements ReportProvider {
  final EpisodeDetailEntryIntent intent;
  final EpisodeDetailService service;
  final HighEpisodeReportAdapter adapter;
  final AnalysisFacade Function() facadeProvider;

  const HighEpisodeReportProvider({
    required this.intent,
    required this.service,
    required this.facadeProvider,
    this.adapter = const HighEpisodeReportAdapter(),
  });

  @override
  String get id => 'episode_detail.high_episode.report';

  @override
  String get title => EpisodeDetailL10nResolver.fallback.highEpisodeReportTitle;
  ...
  static ReportContext defaultContext({
    ...
    return ReportContext(
      range: ReportDateRange(
        start: anchor.subtract(const Duration(days: 14)),
        end: anchor,
      ),
