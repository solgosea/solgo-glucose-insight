...
class AapsXdripOutputEvidenceProbe implements StatusProbeDriver {
  ...
  final definition = const StatusProbeDefinition(
    id: StatusProbeId('aaps.xdrip.output_evidence'),
    suiteId: 'aaps',
    label: 'xDrip+ output evidence',
  ...
  Future<StatusProbeResult> run(StatusProbeContext context) async {
    if (!source.isSupported) {
      return probeResult(
        ...
        summary: 'AAPS downstream evidence is only supported on Android.',
        confidence: 0,
    ...
    if (!snapshot.receiverConfigured || !snapshot.evidenceObserved) {
      return probeResult(
        ...
        observedAt: context.now,
        summary: 'No AAPS downstream evidence from xDrip+ has been observed.',
        confidence: 0,
        signals: [
          signal(
            'Receiver',
