...
class JugglucoNightscoutDelayRule implements StatusMetricRule {
  ...
  Future<StatusRuleResult> evaluate(StatusAnalysisContext context) async {
    const ruleId = StatusRuleId('juggluco.nightscout_delay');
    ...
    final level = comparison.focus == JugglucoChainFocus.xdripToNightscout
        ? StatusLevel.watch
        : StatusLevel.healthy;
    final metric = StatusMetric(
      id: JugglucoMetricIds.nightscoutDelay,
      label: 'xDrip+ -> Nightscout upload',
      valueLabel: comparison.focus == JugglucoChainFocus.xdripToNightscout
          ? 'Cloud behind'
          : 'No cloud gap',
      level: level,
    ...
    return StatusRuleResult(
      ...
      affectsComponentLevel:
          comparison.focus == JugglucoChainFocus.xdripToNightscout,
    );
  }
}
